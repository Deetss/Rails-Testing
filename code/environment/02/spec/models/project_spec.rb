...
RSpec.describe Project do
  ...
  describe "completion" do
    ...
    describe "with a task" do
      let(:project) { FactoryGirl.build_stubbed(:project, tasks: [task]) }
      let(:task) { FactoryGirl.build_stubbed(:task) }
  ...
  describe "estimates" do
    ...
    let(:newly_done) { FactoryGirl.build_stubbed(:task, :newly_complete) }
    let(:old_done) { FactoryGirl.build_stubbed(
        :task, :long_complete, :small) }
    let(:small_not_done) { FactoryGirl.build_stubbed(:task, :small) }
    let(:large_not_done) { FactoryGirl.build_stubbed(:task, :large) }
  ...
  describe "task order" do
    ...
    it "gives me the order of the next task in a project as one more than the most" do
